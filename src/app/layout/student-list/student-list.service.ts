...
export class StudentListService {
  ...
  getStudentList(prepareData): Observable<Array<Student>>{
    const url = environment.apiUrlIp + `/api/student/list`;
    const institute: Institute = this.instituteService.getInstitute();
    const { InstituteID, InstituteUserID } = institute;
    const params = {...prepareData, InstituteID};
    console.log(params);
    return this.http.post<Array<Student>>(url, params)
